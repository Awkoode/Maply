...
function inferUf(row) {
  ...
  if (loc.includes('florianopolis') || loc.includes('florianópolis')) return 'SC';
  if (loc.includes('sao paulo') || loc.includes('são paulo')) return 'SP';
...
function buildPremiumReport(rows, selectedUf) {
  ...
  const estados = Object.keys(UF_NAMES)
  ...
  const uf = requested && UF_NAMES[requested]
    ? requested
    : (estadosComDados.has('SC') ? 'SC' : 'SC');
...
module.exports = { buildPremiumReport, UF_NAMES };
